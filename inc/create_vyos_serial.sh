...
for r in $(seq $first_router $last_router); do 
	ansible-playbook -i inventories/inventory${provider}.yaml vyos_upgrade_fast.yml -e "release_type=$release_type" "-l p${provider}r${r}v"
done
...
for r in $(seq $first_router $last_router); do 
    ansible-playbook -i inventories/inventory${provider}.yaml vyos_configure_fast.yml "-l p${provider}r${r}v"
done
